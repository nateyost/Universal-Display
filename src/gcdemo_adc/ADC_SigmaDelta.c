...
void ADC_SD_Init()
{
    ...
    //offset measurements (so we get proper gain and offset calibration values to use later).
        ...
        SD1CON1bits.PWRLVL = 0; // Low power mode
        ...
        SD1CON1bits.SDGAIN = 2; // Gain is 4:1
        ...
        SD1CON3bits.SDCH = 0;   // Channel 0
        SD1CON3bits.SDCS = 1;   // Clock Source is a 8 MHz FRC
        SD1CON3bits.SDOSR = 3;  // Oversampling Ratio (OSR) is 1024 (best quality)
        SD1CON3bits.SDDIV = 2;  // Input Clock Divider is 2 (SD ADC clock is 4MHz)
    ...
    //we have establish gain/offset calibration values for the ADC).
        SD1CON1bits.SDON = 0;    //Setup Sigma Delta ADC Module for normal reads on CH0
        SD1CON3bits.SDCH = 0;
        SD1CON1bits.SDON = 1;
